...
###################
## below block to get the build defination using build name
###################
...
def get_def_id(azdevopsUser,azdevopsPass,azDefUrl,destroy_pipeline_name):
    ...
    try:
        ...
        print("ERROR - Call Azure DevOps API for Build defination finished with issues....", response)
        print("ERROR - Status Code : ", response.status_code)
        print("ERROR - Reason :", response.reason)


def #{TRIGGER_FUNCTION_ENTRYPOINT}#(event, context):
    print ('Invocating install stage')
    gcp_project_id = "#{GCP_PROJECT_ID}#"
    secret_id = "sm-{}-{}-{}".format(gcp_project_id,'#{GCP_SM_ENV_NAME}#','#{GCP_CS_SM_NAME}#')
    ...
    data = '{"templateParameters": {"action": "install_composer"}, "variables": {}}'
    head = { 'Accept': 'application/json', 'Content-Type': 'application/json'}
